...
class AudioPlayer:
    ...
    async def sync_audio_and_gif(self, audio_file, gif_path):
        try:
            ...
            # Set a fixed duration for testing without audio
            duration_task = asyncio.create_task(asyncio.sleep(5))  # 5 seconds duration
            gif_task = asyncio.create_task(self.display.update_gif(gif_path))

            await asyncio.gather(duration_task, gif_task)
